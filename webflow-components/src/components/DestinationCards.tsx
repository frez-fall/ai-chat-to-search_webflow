...
import type { DestinationRecommendation, GroupedDestinations } from '../types/destinations.ts';
...
export default function DestinationCards({ 
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Destination cards */}
      {destinations && destinations.categories.map((categoryGroup:any) => (
        <div key={categoryGroup.category} className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {categoryGroup.destinations.map((destination:any)=> (
              <button
                key={destination.id}
